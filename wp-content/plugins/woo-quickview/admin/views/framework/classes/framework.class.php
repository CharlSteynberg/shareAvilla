 ...
 *
 */
class SP_WQV_Framework extends SP_WQV_Framework_Abstract {
	...
	public function __construct( $settings, $options ) {
		...
		if( ! empty( $this->options ) ) {
			...
			$this->get_option = get_option( SP_WQV_OPTION );
	...
	public static function instance( $settings = array(), $options = array() ) {
		if ( is_null( self::$instance ) && SP_WQV_F_ACTIVE_FRAMEWORK ) {
	...
	public function validate_save( $request ) {
		...
		$add_errors = array();
		$section_id = sp_wqv_get_var( 'sp_section_id' );
		...
		if ( isset( $request['resetall'] ) ) {
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'woo-quick-view' ), 'updated' );
			return;
		}
		...
		if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
			...
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'woo-quick-view' ), 'updated' );
		}
		...
		set_transient( 'sp_wqv_framework_transient', array( 'errors' => $add_errors, 'section_id' => $section_id ), $transient_time );
	...
	public function field_callback( $field ) {
		...
		echo sp_wqv_add_element( $field, $value, $this->unique );
	}
	...
	public function admin_page() {
		...
		$transient  = get_transient( 'sp_wqv_framework_transient' );
		$has_nav    = ( count( $this->options ) <= 1 ) ? ' sp-show-all' : '';
		$section_id = ( ! empty( $transient['section_id'] ) ) ? $transient['section_id'] : $this->sections[0]['name'];
		$section_id = sp_wqv_get_var( 'sp-section', $section_id );

		echo '<div class="sp-wqv-framework sp-option-framework">';
		...
		echo ( $this->settings['ajax_save'] ) ? '<span id="sp-save-ajax">'. esc_html__( 'Settings saved.', 'woo-quick-view' ) .'</span>' : '';

		submit_button( esc_html__( 'Save Changes', 'woo-quick-view' ), 'primary sp-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'woo-quick-view' ) ) );
		submit_button( esc_html__( 'Restore', 'woo-quick-view' ), 'secondary sp-restore sp-reset-confirm', $this->unique .'[reset]', false );

		if( $this->settings['show_reset_all'] ) {
			submit_button( esc_html__( 'Reset All Options', 'woo-quick-view' ), 'secondary sp-restore sp-warning-primary sp-reset-confirm', $this->unique .'[resetall]', false );
		}

		echo '</fieldset>';
		//echo ( empty( $has_nav ) ) ? '<a href="#" class="sp-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'woo-quick-view' ) .'</a>' : '';
