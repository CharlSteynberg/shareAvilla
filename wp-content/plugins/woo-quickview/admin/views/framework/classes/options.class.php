 ...
 *
 */
abstract class SP_WQV_Framework_Options extends SP_WQV_Framework_Abstract {
	...
	public function element_debug() {
		...
		if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'SP_WQV_OPTIONS_DEBUG' ) && SP_WQV_OPTIONS_DEBUG ) ) {

			$value = $this->element_value();

			$out .= "<pre>";
			$out .= "<strong>". __( 'CONFIG', 'woo-quick-view' ) .":</strong>";
			$out .= "\n";
			ob_start();
			var_export( $this->field );
			$out .= htmlspecialchars( ob_get_clean() );
			$out .= "\n\n";
			$out .= "<strong>". __( 'USAGE', 'woo-quick-view' ) .":</strong>";
			$out .= "\n";
			$out .= ( isset( $this->field['id'] ) ) ? "sp_wqv_get_option( '". $this->field['id'] ."' );" : '';

			if( ! empty( $value ) ) {
				$out .= "\n\n";
				$out .= "<strong>". __( 'VALUE', 'woo-quick-view' ) .":</strong>";
				$out .= "\n";
				ob_start();
				var_export( $value );
				$out .= htmlspecialchars( ob_get_clean() );
		...
		if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'SP_WQV_OPTIONS_DEBUG_LIGHT' ) && SP_WQV_OPTIONS_DEBUG_LIGHT ) ) {
			...
			$out .= ( isset( $this->field['id'] ) ) ? "sp_wqv_get_option( '". $this->field['id'] ."' );" : '';
			$out .= "\n";
			$out .= "<strong>". __( 'ID', 'woo-quick-view' ) .":</strong>";
			$out .= "\n";
		...
		if ( is_array( $this->multilang ) ) {
			...
			foreach ( $this->multilang['languages'] as $key => $val ) {
				...
				if( $key != $this->multilang['current'] ) {
					...
					$class = 'SP_WQV_Framework_Option_' . $this->field['type'];
			...
			$out .= '</fieldset>';
			$out .= '<p class="sp-text-desc">'. sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'woo-quick-view' ), $this->multilang['current'] ) .'</p>';
