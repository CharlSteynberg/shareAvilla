 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SP_WCS_Field_group' ) ) {
	class SP_WCS_Field_group extends SP_WCS_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				...
				array(
					...
					'min'                    => 0,
					'fields'                 => array(),
					'button_title'           => esc_html__( 'Add New', 'woo-category-slider' ),
			...
			if ( ! empty( $this->parent ) && preg_match( '/' . preg_quote( '[' . $this->field['id'] . ']' ) . '/', $this->parent ) ) {
				...
				echo '<div class="spf-notice spf-notice-danger">' . esc_html__( 'Error: Nested field id can not be same with another nested field id.', 'woo-category-slider' ) . '</div>';
				...
				echo '<i class="spf-cloneable-remove spf-confirm fa fa-times" data-confirm="' . esc_html__( 'Are you sure to delete this item?', 'woo-category-slider' ) . '"></i>';
				echo '</div>';
				...
				foreach ( $this->field['fields'] as $field ) {
					...
					SP_WCS::field( $field, $field_default, '_nonce', 'field/group', $field_parent );
				...
				if ( ! empty( $this->value ) ) {
					...
					foreach ( $this->value as $value ) {
						...
						echo ( $title_prefix ) ? '<span class="spf-cloneable-title-prefix">' . $title_prefix . '</span>' : '';
						echo ( $title_auto ) ? '<span class="spf-cloneable-value"></span>' : '';
						echo '</span>';
						echo '</h4>';
						...
						foreach ( $this->field['fields'] as $field ) {
							...
							SP_WCSP::field( $field, $field_value, $field_unique, 'field/group', $field_parent );
				...
				echo '<div class="spf-cloneable-alert spf-cloneable-max">' . esc_html__( 'You can not add more than', 'woo-category-slider' ) . ' ' . $args['max'] . '</div>';
				echo '<div class="spf-cloneable-alert spf-cloneable-min">' . esc_html__( 'You can not remove less than', 'woo-category-slider' ) . ' ' . $args['min'] . '</div>';
