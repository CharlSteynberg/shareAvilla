...
if ( ! class_exists( 'SPWPS_Field_typography' ) ) {
	class SPWPS_Field_typography extends SPWPS_Fields {
		...
		public function render() {
			...
			// echo '<div class="spwps--block spwps--block-backup-font-family hidden">';
			// echo '<div class="spwps--title">' . esc_html__( 'Backup Font Family', 'woo-product-slider' ) . '</div>';
			// echo $this->create_select(
			// apply_filters(
			// 'spwps_field_typography_backup_font_family', array(
			...
			// 'Palatino Linotype',
			// )
			// ), 'backup-font-family', esc_html__( 'Default', 'woo-product-slider' )
			// );
			// echo '</div>';
			// }
			//
			...
			if ( ! empty( $args['text_align'] ) ) {
				...
				echo $this->create_select(
					...
					), 'text-align', esc_html__( 'Default', 'woo-product-slider' )
				);
				echo '</div>';
			}
			...
			if ( ! empty( $args['font_variant'] ) ) {
				...
				echo $this->create_select(
					...
					), 'font-variant', esc_html__( 'Default', 'woo-product-slider' )
				);
				echo '</div>';
			}
			...
			if ( ! empty( $args['text_transform'] ) ) {
				...
				echo $this->create_select(
					...
					), 'text-transform', esc_html__( 'Default', 'woo-product-slider' )
				);
				echo '</div>';
			}
			...
			if ( ! empty( $args['text_decoration'] ) ) {
				...
				echo $this->create_select(
					...
					), 'text-decoration', esc_html__( 'Default', 'woo-product-slider' )
				);
				echo '</div>';
			}
			...
			// 	echo '<div class="spwps--block">';
			// 	echo '<div class="spwps--title">' . esc_html__( 'Word Spacing', 'woo-product-slider' ) . '</div>';
			// 	echo '<div class="spwps--input-wrap">';
			// 	echo '<input type="number" name="' . esc_attr( $this->field_name( '[word-spacing]' ) ) . '" class="spwps--word-spacing spwps--input spwps-input-number" value="' . esc_attr( $this->value['word-spacing'] ) . '" />';
			// 	echo '<span class="spwps--unit">' . esc_attr( $args['unit'] ) . '</span>';
			// 	echo '</div>';
			// 	echo '</div>';
			// }
			...
			// 	echo '<div class="spwps--block spwps--block-preview' . esc_attr( $always_preview ) . '">';
			// 	echo '<div class="spwps--toggle fas fa-toggle-off"></div>';
			// 	echo '<div class="spwps--preview">' . esc_attr( $args['preview_text'] ) . '</div>';
			// 	echo '</div>';
			// }
		...
		public function enqueue() {
			...
			if ( ! wp_script_is( 'spwps-webfontloader' ) ) {
				...
				// SPWPS::include_plugin_file( 'fields/typography/google-fonts.php' );

				// wp_enqueue_script( 'spwps-webfontloader', 'https://cdn.jsdelivr.net/npm/webfontloader@1.6.28/webfontloader.min.js', array( 'spwps' ), '1.6.28', true );
            ...
            'Open Sans'=>[['300','300italic','normal','italic','600','600italic','700','700italic','800','800italic'],['cyrillic-ext','cyrillic','greek-ext','latin-ext','greek','latin','vietnamese']]
				...
				$googlestyles = apply_filters(
					'spwps_field_typography_googlestyles', array(
						'100'       => 'Thin 100',
				...
				wp_localize_script(
					'spwps', 'spwps_typography_json', array(
						'webfonts'      => $webfonts,
		...
		// 	} else {
		// 		SPWPS::include_plugin_file( 'fields/typography/google-fonts.php' );
		// 		$is_google = ( array_key_exists( $this->value['font-family'], spwps_get_google_fonts() ) ) ? true : false;
		// 	}
