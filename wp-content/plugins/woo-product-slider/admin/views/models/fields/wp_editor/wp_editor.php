...
if ( ! class_exists( 'SPWPS_Field_wp_editor' ) ) {
  class SPWPS_Field_wp_editor extends SPWPS_Fields {
    ...
    public function render() {
      ...
      echo ( spwps_wp_editor_api() ) ? '<div class="spwps-wp-editor" data-editor-settings="'. esc_attr( json_encode( $editor_settings ) ) .'">' : '';
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var spwps_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( spwps_wp_editor_api() && class_exists( '_WP_Editors') ) {
        ...
        $defaults = apply_filters( 'spwps_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );
