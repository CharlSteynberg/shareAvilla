  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.esc_html__('Remove','wolverine').'" title="'.esc_html__('Remove','wolverine').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.esc_html__('Add','wolverine').'" title="'.esc_html__('Add','wolverine').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_image( $field, $meta ) {
    ...
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
    echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".esc_html__('Remove Image', 'wolverine')."' type='button'/>";
    else
    echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".esc_html__('Upload Image', 'wolverine')."' type='button'/>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function footer_js(){
    ...
    jQuery(document).ready(function(){
      ...
      function clear_form_meta(){
          ...
          jQuery(".mupload_img_holder").next().next().next().val("<?php esc_html_e('Upload Image','wolverine'); ?>");
